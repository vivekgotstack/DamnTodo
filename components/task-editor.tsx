...
export function TaskEditor({ task, initialScheduledAt, defaultDuration, alarmsAvailable, onAlarmUnavailable, onClose, onSave }: {
  ...
  const alarmLocked = Boolean(task?.alarmModeLocked);
  ...
  const scheduledEnd = useMemo(() => {
    ...
    const end = new Date(new Date(draft.scheduledAt).getTime() + draft.duration * 60_000);
    if (Number.isNaN(end.getTime())) return null;
    return new Intl.DateTimeFormat("en-US", { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(end);
  }, [draft.duration, draft.scheduledAt]);
  ...
  const setKind = (kind: DraftKind) => setDraft((current) => ({
    ...
    dueAt: kind === "goal" ? (current.kind === "goal" && current.dueAt ? current.dueAt : endDateAfterMonths(6, current.availableFrom)) : "",
              ...
              <>
                ...
                <div className={`alarm-builder ${draft.alarmMode === "strict" ? "is-strict" : ""}`}>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    {alarmsAvailable && draft.alarmMode !== "none" && <><p className="hourly-policy sm:col-span-2"><AlarmClock className="size-4" />Quiet during each session. If unfinished, reminders repeat hourly until 11:59 PM.</p><div className="grid gap-2 sm:col-span-2"><Label htmlFor="goal-backlog-time">Next-day retry time</Label><Input id="goal-backlog-time" type="time" value={draft.backlogAlarmTime} onChange={(event) => update("backlogAlarmTime", event.target.value)} className="h-11 border-white/12 bg-[#0b192c] text-white [color-scheme:dark]" /><small className="text-slate-500">After midnight, unfinished sessions move to backlog and restart here. Default is 09:00.</small></div></>}
                  </div>
                  {alarmsAvailable && draft.alarmMode === "strict" && <p className="strict-explainer"><AlarmClock className="size-4" />Red mode sounds every hour after the session ends. Its mode locks when this roadmap is created.</p>}
                </div>
              </>
            ) : (
              <>
                ...
                <div className="goal-builder"><div className="goal-builder-head"><span className="goal-builder-icon"><Clock3 className="size-4" /></span><div><strong>Time block</strong><p>Choose how long the work takes. If you schedule it, DamnTodo calculates the finish time and starts follow-ups then.</p></div></div><div className="grid gap-4 sm:grid-cols-2"><div className="grid gap-2"><Label>Work length</Label><Select value={String(draft.duration)} onValueChange={(value) => update("duration", Number(value))}><SelectTrigger aria-label="Work length" className="h-11 w-full border-white/12 bg-[#0b192c] text-white"><SelectValue /></SelectTrigger><SelectContent className="border-white/12 bg-[#0b192c] text-slate-100">{DURATIONS.map((minutes) => <SelectItem key={minutes} value={String(minutes)}>{formatDuration(minutes)}</SelectItem>)}</SelectContent></Select></div><div className="grid gap-2"><Label>Priority</Label><Select value={draft.priority} onValueChange={(value) => update("priority", value as Priority)}><SelectTrigger aria-label="Priority" className="h-11 w-full border-white/12 bg-[#0b192c] text-white capitalize"><SelectValue /></SelectTrigger><SelectContent className="border-white/12 bg-[#0b192c] text-slate-100">{(["low", "medium", "high"] as Priority[]).map((priority) => <SelectItem key={priority} value={priority} className="capitalize">{priority}</SelectItem>)}</SelectContent></Select></div><div className="grid gap-2 sm:col-span-2"><Label htmlFor="task-schedule">Schedule start <span className="font-normal text-slate-500">optional</span></Label><Input id="task-schedule" type="datetime-local" value={draft.scheduledAt} min={task ? undefined : localDateTime()} onChange={(event) => update("scheduledAt", event.target.value)} className="h-11 border-white/12 bg-[#0b192c] text-white [color-scheme:dark]" />{scheduledEnd && <small className="text-sky-100/65">Ends {scheduledEnd} · based on the selected work length</small>}</div></div></div>
                {alarmsAvailable ? <div className={`alarm-builder ${draft.alarmMode === "strict" ? "is-strict" : ""}`}><div className="grid gap-4"><div className="grid gap-2"><Label>After-session follow-up</Label><Select disabled={!draft.scheduledAt || alarmLocked} value={alarmLocked ? draft.alarmMode : draft.scheduledAt ? draft.alarmMode : "none"} onValueChange={(value) => { const mode = value as AlarmMode; update("alarmMode", mode); update("reminderMinutes", mode === "none" ? null : 0); }}><SelectTrigger className="h-11 w-full border-white/12 bg-[#0b192c] text-white"><SelectValue /></SelectTrigger><SelectContent className="border-white/12 bg-[#0b192c] text-slate-100"><SelectItem value="none">No follow-up</SelectItem><SelectItem value="gentle">Gentle · hourly message</SelectItem><SelectItem value="strict">Red Mode · audible hourly alarm</SelectItem></SelectContent></Select>{alarmLocked && <small className="alarm-lock-note"><LockKeyhole className="size-3.5" />This alarm mode is locked for the life of this schedule.</small>}</div>{draft.scheduledAt && draft.alarmMode !== "none" && <p className="hourly-policy"><AlarmClock className="size-4" />No disturbance during the {formatDuration(draft.duration)} session. Follow-ups begin when it ends and repeat hourly until 11:59 PM.</p>}{draft.alarmMode !== "none" && (draft.scheduledAt || task?.status === "backlog") && <div className="grid gap-2"><Label htmlFor="backlog-alarm-time">Next-day retry time</Label><Input id="backlog-alarm-time" type="time" value={draft.backlogAlarmTime} onChange={(event) => update("backlogAlarmTime", event.target.value)} className="h-11 border-white/12 bg-[#0b192c] text-white [color-scheme:dark]" /><small className="text-slate-500">After 11:59 PM it moves to backlog and hourly follow-ups restart here. Default is 09:00.</small></div>}</div></div> : <AlarmInstallGate onClick={onAlarmUnavailable} />}
              </>
            )}
