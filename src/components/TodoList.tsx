...
function TodoList() {
  ...
  useEffect(() => {
    try {
      ...
      setTodos(parsed.map((t: any) => ({ ...t, id: new Date(t.id) })));
    } catch (e) {
  ...
  // Delete a todo
  const handleDelTodo = (id: Date) => {
    setTodos((prev) => {
      const updated = prev.filter((t) => t.id.getTime() !== id.getTime());
      localStorage.setItem("todos", JSON.stringify(updated));
      return updated;
    });
  };

  const toggleMode = (id: Date) => {
  setTodos((prev) => {
    const updated = prev.map((t) =>
      t.id.getTime() === id.getTime()
        ? { ...t, mode: t.mode === "edit" ? "save" : "edit" } as TodoProps
        : t
    );
    localStorage.setItem("todos", JSON.stringify(updated));
    return updated;
  ...
  // Edit todo message
  const handleEditTodo = (id: Date, msg: string) => {
    setTodos((prev) => {
      const updated = prev.map((t) =>
        t.id.getTime() === id.getTime() ? { ...t, msg } : t
      );
      localStorage.setItem("todos", JSON.stringify(updated));
      return updated;
    });
  ...
  return (
    <div className="flex w-full min-h-screen items-center flex-col justify-center bg-gray-700">
      ...
      {todos.map((todo) => (
        <div
          key={todo.id.getTime()}
          className="mt-5 w-full wrap-break-word bg-gray-600 rounded-lg p-5 border border-gray-800 flex flex-col justify-between hover:border-black"
        >
          {todo.mode === "edit" ? (
            <input
